...
const navItems: NavItem[] = [
  {
    ...
    panel: [
      ...
      },
      {
        ...
        items: [
          { label: "Phoenix, USA", to: "/about/usa" },
          { label: "Frankfurt, Germany", to: "/about/germany" },
          { label: "Helsinki, Finland", to: "/about/finland" },
          { label: "Riyadh, Saudi Arabia", to: "/about/saudi-arabia" },
          { label: "Melbourne, Australia", to: "/about/australia" },
          { label: "Delhi, India", to: "/about/india" },
          { label: "Sydney, Australia", to: "/about/sydney" },
        ],
      },
    ],
  },
      ...
      >
        <div className="flex items-center justify-between px-8 md:px-14 py-3.5">
          ...
          <Link to="/" onClick={closePanel} className="shrink-0 z-10 cursor-pointer">
            {/* Shadow on wrapper keeps the logo bitmap sharp: stacking drop-shadow + invert on one <img> blurs edges */}
            <span
              className="inline-flex [filter:drop-shadow(0_2px_10px_rgba(0,0,0,0.65))]"
            ...
            >
              <img
                ...
                decoding="async"
                draggable={false}
                style={lightStyle ? { filter: "brightness(0) invert(1)" } : undefined}
              />
            </span>
          </Link>
        ...
        {/* Mobile accordion menu — max height = viewport minus top bar so bottom links (e.g. AI, Cyber Security) can scroll into view */}
        <div
          className="md:hidden overflow-hidden transition-all duration-400"
          style={{
            maxHeight: mobileOpen ? "calc(100dvh - 5.5rem)" : "0",
            background: "rgba(255,255,255,0.99)",
            borderTop: "1px solid rgba(0,0,0,0.06)",
          }}
        >
          <div className="max-h-[calc(100dvh-5.5rem)] overflow-y-auto overscroll-contain px-6 pt-2 space-y-1 pb-[max(2rem,env(safe-area-inset-bottom,0px))]">
            {navItems.map((item) =>
              item.panel ? (
                <div key={item.to}>
                  <Link
                  ...
                  <div className="pl-4 pb-1">
                    {item.panel.map((section) => (
                      <div key={section.heading} className="mt-3 first:mt-0">
                        <p className="py-1.5 text-gray-400 text-[10px] tracking-[0.22em] uppercase border-b border-gray-100">
                          {section.heading}
                        </p>
                        {section.items.map((sub) => (
                          <div key={sub.to}>
                            <Link
                              to={sub.to}
                              className="block py-2 text-gray-600 text-xs border-b border-gray-50 cursor-pointer hover:text-teal-600"
                              onClick={() => setMobileOpen(false)}
                            >
                              {sub.label}
                            </Link>
                              ...
                              <div className="pl-3 border-l border-gray-100 ml-1">
                                {sub.children.map((child) => (
                                  <Link
                                    key={child.to}
                                    to={child.to}
                                    className="block py-2 text-gray-500 text-[11px] border-b border-gray-50 cursor-pointer hover:text-teal-600"
                                    onClick={() => setMobileOpen(false)}
                                  >
                                    {child.label}
                                  </Link>
